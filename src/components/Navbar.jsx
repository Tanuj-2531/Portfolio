...
const Navbar = () => {
  ...
  return (
    <div className={`${theme === 'dark' ? 'bg-[#1e1e1e]/30 text-white' : 'bg-[#e5e3d9]/30 text-[#111]'} backdrop-blur-md shadow-md flex items-center justify-between z-50 rounded-full fixed h-15 top-4 left-15 right-15 md:left-25 md:right-25`}>
      <div className='p-4 md:p-15 flex items-center'>
          <img className={`h-10 md:h-25 ${theme === "dark" ? "invert-0" : "invert"}`} src="/logo.png" alt="" />
      ...
      {isOpen && (
        <div className={`${theme === 'dark' ? 'bg-[#1e1e1e]/30 text-white shadow-white/30' : 'bg-[#e5e3d9]/30 text-[#111] shadow-black/30'} absolute top-18 right-5 flex flex-col gap-4 p-5 rounded-xl shadow-lg font-bold md:hidden`}>
